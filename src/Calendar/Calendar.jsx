...
const Calendar = ({pageContent}) => {
    ...
    useEffect(() => {
        ...
        axios.get("/api/calendar/events").then(({data}) => {
            // Filter events to only show those created by or shared with the current user
            const userEvents = data.filter(event => 
                event.createdBy?._id === auth?._id || // Created by current user
                event.invitations?.includes(auth?._id) // Or user is invited
            );
