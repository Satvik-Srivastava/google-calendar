...
const CalendarSidebar = (props) => {
    ...
    useEffect(() => {
        if (events.length > 0) {
            ...
            // Filter events where the current user is invited
            setInvitedMe(events.filter(evt => evt.invitations?.some(userId => userId === authState?.auth?._id)))
        }
    }, [events, authState?.auth?._id])
            ...
            <div className="sidebar-scroll">
                ...
                <Accordion openIds={expandItems}>
                    ...
                    <Accordion.Item dataId={2}
                        ...
                        <div className="accordion-content">
                            {sortByDate(events.filter(evt => 
                                // Show only events created by the current user
                                evt.createdBy?._id === authState?.auth?._id
                            )).slice(0, accItemShowContentLen[2]).map(evt => (
                                        ...
                                        <div className="ml-2">
                                            <button
                                                onClick={(e)=>{
                                                    e.stopPropagation();
                                                    e.preventDefault();
                                                    // remove event locally from context so it disappears from sidebar and timeline
                                                    if(typeof setEvents === "function"){
                                                        setEvents(prev => prev.filter(item => item._id !== evt._id))
                                                    }
                                                }}
                                                title="Delete event"
                    ...
                    <Accordion.Item dataId={3}
                        header={(isOpen) => (
                            <div onClick={() => handleToggle(3)} className="accordion-header flex items-center justify-between">
                                <h4 className="">Invited Events</h4>
                                {isOpen
                                    ? <BiChevronDown className="text-xs text-gray-500"/>
                                    : <BiChevronRight className="text-xs text-gray-500"/>
                                }
                        ...
                        <div className="accordion-content">
                            {sortByDate(invitedMe).slice(0, accItemShowContentLen[3] || 10).map(evt => (
                                <div className="accordion-li mt-1" key={evt._id} onClick={()=>handleOpenEventDetailRoute(evt._id)}>
                                    <h4 className="flex items-center justify-between w-full">
                                        <div className="flex items-center">
                                            <div className="col-span-1">
                            ...
                            {invitedMe.length > 10 && (
                                <div className="accordion-li hover:bg-blue-100 p-1 rounded"
                                     onClick={(e) => handleExpandAccContent(e, 3, invitedMe.length)}>
                                    <div className="ml-4 flex items-center justify-between">
                                        <label className="font-medium">
                                            Show {accItemShowContentLen[3] !== invitedMe.length ? " more" : " less"}</label>
                                        <BiChevronDown/>
                                    </div>
                                </div>
                            )}
